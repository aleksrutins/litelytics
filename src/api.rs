use rocket::{http::CookieJar, response::status::Unauthorized, Route, State, serde::json::Json};
use sqlx::{Pool, Postgres};

use crate::auth::{ensure_authenticated, self};
use crate::models::{Site, SiteData, UserSite, Visit, CreateVisitMeta, CreateVisitRequest, CreateVisit};
...
pub async fn sites(
    ...
    let sites = sqlx::query_as!(
        ...
        cookies
            .get_private("user_id")
            .map(|uid| uid.value().parse::<i32>().unwrap_or(-1))
            .unwrap_or(-1)
    )
...
#[post("/track", data = "<input>")]
pub async fn track(pool: &State<Pool<Postgres>>, cookies: &CookieJar<'_>, meta: CreateVisitMeta, input: Json<CreateVisitRequest>) {
    if !ensure_authenticated(cookies) {
        
    }
    let create_visit = CreateVisit {
        site: meta.site,
        ip: meta.ip,
...
pub async fn site_data(
    ...
    serde_json::to_string(&SiteData { site, visits })
