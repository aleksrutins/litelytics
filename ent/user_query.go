...
func (uq *UserQuery) QuerySites() *SiteQuery {
	query := &SiteQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withSites; query != nil {
		if err := uq.loadSites(ctx, query, nodes,
			func(n *User) { n.Edges.Sites = []*Site{} },
			func(n *User, e *Site) { n.Edges.Sites = append(n.Edges.Sites, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadSites(ctx context.Context, query *SiteQuery, nodes []*User, init func(*User), assign func(*User, *Site)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.SitesTable)
		s.Join(joinT).On(s.C(site.FieldID), joinT.C(user.SitesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.SitesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.SitesPrimaryKey[0]))
